...
fun run(testCaseNum: Int) {
    ...
    for (i in 60 downTo 0) {
        var maxInd = -1
        for (j in k until N) if ((maxInd == -1 || A[j] > A[maxInd]) && (A[j] shr i) and 1L == 1L) maxInd = j
        if (maxInd == -1) continue
        A[k] = A[maxInd].also{A[maxInd] = A[k]}
        for (j in 0 until N) if (j != k && (A[j] shr i) and 1L == 1L) A[j] = A[j] xor A[k]
        k++
    }
    Out.println(A.reduce{res, a -> res xor a})
}
