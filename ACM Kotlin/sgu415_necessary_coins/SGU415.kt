...
fun run(testCaseNum: Int) {
    val N = In.nextInt()
    val X = In.nextInt()
    val A = IntArray(N, {In.nextInt()})
    val ans = ArrayList<Int>()
    for (i in 0 until N) {
        val dp = BooleanArray(X + 1, {it == 0})
        for (j in 0 until N) if (i != j) for (k in X - A[j] downTo 0) if (dp[k]) dp[k + A[j]] = true
        if (!dp[X]) ans.add(A[i])
    }
    Out.println(ans.size)
    for (a in ans) Out.print("$a ")
    Out.println()
}
