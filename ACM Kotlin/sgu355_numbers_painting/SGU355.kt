// http://codeforces.com/problemsets/acmsguru/problem/99999/355
import java.io.*
import java.math.*
import java.util.*
...
fun run(testCaseNum: Int) {
    var N = In.nextInt()
    var color = IntArray(N + 1)
    color[1] = 1
    for (i in 2..N) color[i] = (1..(Math.floor(Math.sqrt(i.toDouble())).toInt())).fold(0) { c, j -> if (i % j == 0) Math.max(c, Math.max(color[j] + 1, color[i / j] + 1)) else c }
    Out.println((1..N).fold(0) { max, i -> Math.max(max, color[i]) })
    for (i in 1..N) Out.print("${color[i]} ")
    Out.println()
}
