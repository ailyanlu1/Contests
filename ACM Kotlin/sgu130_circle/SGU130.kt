// http://codeforces.com/problemsets/acmsguru/problem/99999/130
import java.io.*
import java.math.*
import java.util.*
...
fun run(testCaseNum: Int) {
    var N = In.nextInt()
    var comb = LongArray(N + 1)
    comb[0] = 1
    comb[1] = 1
    for (i in 2..N) for (j in 0 until i) comb[i] += comb[j] * comb[i - j - 1];
    Out.println("${comb[N]} ${N + 1}")
}
