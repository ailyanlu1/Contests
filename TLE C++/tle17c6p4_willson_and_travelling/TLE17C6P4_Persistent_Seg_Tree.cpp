...
#define MAXN 200005
#define MAXC 500005
#define MAXNODES MAXN * 120
 
int N, L[MAXNODES], R[MAXNODES], VAL[MAXNODES], root[MAXC], curNode = 0;
vector<pii> adj[MAXC];
vector<pii> P;
...
int makeNode(int l, int r) {
    assert(++curNode < MAXNODES);
    VAL[curNode] = VAL[L[curNode] = l] + VAL[R[curNode] = r];
    return curNode;
}

int build(int cL, int cR) {
    if (cL == cR) return makeNode(0);
    int m = cL + (cR - cL) / 2;
...
int update(int cur, int cL, int cR, int ind, int val) {
    ...
    if (cL == cR) return makeNode(VAL[cur] + val);
    int m = cL + (cR - cL) / 2;
...
//    freopen("out.txt", "w", stdout);
    ...
    For(i, 1, MAXC) {
        root[i] = root[i - 1];
        for (pii &p : adj[i]) root[i] = update(root[i], 0, MAXC, p.f, p.s);
    }
    int cnt = 0;
    for (pii &p : P) if ((query(root[p.f], 0, MAXC, 0, p.s) > 0) + (query(root[p.f + 1], 0, MAXC, 0, p.s) > 0) + (query(root[p.f], 0, MAXC, 0, p.s + 1) > 0) + (query(root[p.f + 1], 0, MAXC, 0, p.s + 1) > 0) == 1) cnt++;
    writeln(cnt);
    return 0;
}
