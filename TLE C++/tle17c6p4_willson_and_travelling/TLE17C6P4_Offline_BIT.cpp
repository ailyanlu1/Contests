...
int N, BIT[MAXC], cnt[MAXN * 16], curInd = 0;
vector<pii> updates[MAXC], queries[MAXC], P;
vector<int> ind;

void addQueries(int x, int y) {
    queries[x].eb(y, curInd);
    queries[x + 1].eb(y, curInd);
    queries[x].eb(y + 1, curInd);
    queries[x + 1].eb(y + 1, curInd);
    P.eb(x, y);
    ind.pb(curInd++);
}
...
//    freopen("out.txt", "w", stdout);
    ...
    FOR(i, N * 16) cnt[i] = 0;
    FOR(i, MAXC) {
        for (pii &p : updates[i]) update(p.f, p.s);
        for (pii &p : queries[i]) cnt[p.s] += query(p.f) > 0;
    }
    sort(all(ind), [&](const int &i, const int &j) { return P[i] < P[j]; });
    ind.erase(unique(all(ind), [&](const int &i, const int &j) { return P[i] == P[j]; }), ind.end());
    int ans = 0;
    for (int i : ind) ans += cnt[i] == 1;
    writeln(ans);
    return 0;
}
