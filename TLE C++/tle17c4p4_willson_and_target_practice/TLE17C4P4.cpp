...
//    freopen("out.txt", "w", stdout);
    ...
    read(N, K);
    height = slope * K / 2.0;
    FOR(i, N) read(P[i].x, P[i].y);
    FOR(_, 2) {
        For(y, -MAXC - K, MAXC + K) {
            FOR(i, N) {
                if (P[i].y <= y + height + EPS && P[i].y >= y) {
                    events.eb(P[i].x - (y + height - P[i].y) / slope - EPS, -1);
                    events.eb(P[i].x + (y + height - P[i].y) / slope + EPS, 1);
                }
            }
            sort(all(events));
            int cnt = 0;
            for (pair<double, int> e : events) MAX(ans, cnt -= e.s);
            assert(cnt == 0);
            events.clear();
        }
        FOR(i, N) P[i].y = -P[i].y;
