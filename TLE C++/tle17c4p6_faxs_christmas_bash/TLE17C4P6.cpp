...
const char*_delimiter=" ";int _cur,_outputPtr=0,_numPtr=0,_precision=6,_width=0,_tempOutputPtr=0,_cnt;uull _precisionBase=1000000;
...
i128 ternary_search(i128 (*f)(i128), i128 lo, i128 hi) {
    i128 mid;
    while (lo < hi) {
        mid = lo + (hi - lo) / 2;
...
FenwickTree<i128> cube(MAXX * 2), quad(MAXX * 2), lin(MAXX * 2), con(MAXX * 2);

i128 f(i128 x) {
    int below = (int) (x / 1000 - (x < 0 && x % 1000 ? 1 : 0));
    int above = (int) (x / 1000 + (x > 0 && x % 1000 ? 1 : 0));
    i128 ret = ((((con.rsq(1, below + MAXX) - con.rsq(above + MAXX, MAXX * 2)) * x + (lin.rsq(above + MAXX, MAXX * 2) - lin.rsq(1, below + MAXX))))
...
int main() {
    ...
    FOR(i, N) {
        read(xx);
        x = (i128) xx * 1000;
        cube.update(xx + MAXX, (i128) 1 * (i128) x * (i128) x * (i128) x);
        quad.update(xx + MAXX, (i128) 3 * (i128) x * (i128) x);
        lin.update(xx + MAXX, (i128) 3 * (i128) x);
        con.update(xx + MAXX, (i128) 1);
        writeln(((ld) (ternary_search(f, -15000000, 15000000))) / 1000);
    }
    return 0;
}
