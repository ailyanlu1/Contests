...
// THIS ONLY WORKS ON g++, NOT clang++ (variable length arrays)
const int MAXN = 9, MAXA = 5, MAXPROD = pow(MAXA, MAXN);

int N, A[MAXN], prod = 1, sufProdEx[MAXN], sufProdIn[MAXN];
ll x[MAXPROD], dp[2][MAXPROD];
...
int main() {
    ...
    FOR(i, N) {
        FOR(j, prod) {
            dp[i % 2][j] = dp[1 - i % 2][j];
            if (j - sufProdEx[i] >= 0 && j / sufProdIn[i] == (j - sufProdEx[i]) / sufProdIn[i]) dp[i % 2][j] += dp[1 - i % 2][j - sufProdEx[i]];
            if (j + sufProdEx[i] < prod && j / sufProdIn[i] == (j + sufProdEx[i]) / sufProdIn[i]) dp[i % 2][j] += dp[1 - i % 2][j + sufProdEx[i]];
        }
    }
    FOR(j, prod) printf("%lld\n", dp[(N - 1) % 2][j] - x[j]);
    return 0;
}
