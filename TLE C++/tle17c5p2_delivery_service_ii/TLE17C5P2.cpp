...
int N, D, X[MAXN], max1 = -INT_INF, max2 = -INT_INF, min1 = INT_INF, min2 = INT_INF;
...
int main() {
    ...
    For(i, 1, D + 1) {
        ri(p1);
        ri(p2);
        if (X[p1] < X[p2]) dec = false;
        else if (X[p1] > X[p2]) inc = false;
        MAX(max1, X[p1]);
        MAX(max2, X[p2]);
        MIN(min1, X[p1]);
        MIN(min2, X[p2]);
    }
    int d1 = 2 * max(max1, max2) - min1 - min2;
    int d2 = max1 + max2 - 2 * min(min1, min2);
    if (inc) printf("%d\n", max2 - min1);
