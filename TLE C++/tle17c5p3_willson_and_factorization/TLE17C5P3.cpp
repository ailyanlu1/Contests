...
int multInv(int a, int n) {
    int x, y;
    if (gcd(a, n, x, y) != 1) return 0;
    return (x % n + n) % n;
}

int N;
set<int> units, nonunits, irreducibles, primes;

int main() {
    ri(N);
    FOR(u, N) {
        int v = multInv(u, N);
        if (v != 0) units.insert(u);
        else nonunits.insert(u);
    }
    for (int i : nonunits) {
        if (i == 0) continue;
        ...
        if (isIrreducible) irreducibles.insert(i);
    }
    for (int p : nonunits) {
        if (p == 0) continue;
        bool isPrime = true;
        For(a, 1, N) {
            For(b, a, N) {
                int x, y, g = gcd(p, N, x, y);
                if (a * b % g == 0 && a % g != 0 && b % g != 0) {
                    isPrime = false;
                    break;
                }
            }
        ...
        if (isPrime) primes.insert(p);
    }
    printf("Units:\n");
    for (int x : units) printf("%d\n", x);
    printf("Irreducibles:\n");
    for (int x : irreducibles) printf("%d\n", x);
    printf("Primes:\n");
    for (int x : primes) printf("%d\n", x);
    return 0;
}
