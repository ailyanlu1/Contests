package ecoo17r3p3_region_selection;
...
public class ECOO17R3P3 {
    private static ECOO17R3P3 o = new ECOO17R3P3();
    public class Reader {
    ...
    private static Point2D[] loc;
    
    public static void run(int testCaseNum) throws IOException {
        int N = in.nextInt();
        loc = new Point2D[N];
        for (int i = 0; i < N; i++) {
            loc[i] = new Point2D(in.nextInt(), in.nextInt());
        }
        if (N == 1) {
            out.println(0);
            return;
            ...
            for (int j = 0; j < N; j++) {
                ...
                for (int k = 0; k < N; k++) {
                    if (a * loc[k].x + b * loc[k].y + c > 0) {
                        ...
                        if (b == 0) {
                            if (loc[k].y >= loc[i].y) ans1.add(k);
                            else ans2.add(k);
                        } else {
                            if (loc[k].x >= loc[i].x) ans1.add(k);
                            else ans2.add(k);
                        }
                    } else {
                        ans2.add(k);
                ...
                ans = Math.min(ans, solve(ans1) + solve(ans2));
            }
        }
        out.println((int) Math.round(ans));
    }
    
    private static double solve(ArrayList<Integer> A) {
