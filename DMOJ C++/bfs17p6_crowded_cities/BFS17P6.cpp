...
int N, PREV[MAXN], BIT[MAXLWH][MAXLWH], ORD[MAXN];
ll VAL[MAXN];

bool cmp(int i, int j) {
    return VAL[i] == VAL[j] ? ORD[i] < ORD[j] : VAL[i] < VAL[j];
}

void update(int i, int j, int ind) {
    for (int x = i; x < MAXLWH; x += x & -x) for (int y = j; y < MAXLWH; y += y & -y) if (cmp(BIT[x][y], ind)) BIT[x][y] = ind;
}

int query(int i, int j) {
    int ret = N;
    assert(i < MAXLWH && j < MAXLWH && i > 0 && j > 0);
    for (int x = i; x > 0; x -= x & -x) for (int y = j; y > 0; y -= y & -y) if (cmp(ret, BIT[x][y])) ret = BIT[x][y];
    return ret;
}
...
//    freopen("out.txt", "w", stdout);
    ...
    read(N);
    FOR(x, MAXLWH) FOR(y, MAXLWH) BIT[x][y] = N;
    int D[3];
    FOR(i, N) {
        read(D[0], D[1], D[2], B[i].P);
        sort(D, D + 2);
        B[i].L = D[0];
        B[i].W = D[1];
        B[i].H = D[2];
        B[i].ind = i;
        PREV[i] = N;
        assert(B[i].L != 0 && B[i].W != 0 && B[i].H != 0 && B[i].P != 0);
    }
    sort(B, B + N);
    FOR(i, N) ORD[B[i].ind] = i;
    ORD[N] = N;
    VAL[N] = 0;
    FOR(i, N) {
        int q = query(B[i].L, B[i].W);
        PREV[B[i].ind] = q;
        VAL[B[i].ind] = VAL[q] + B[i].P;
        update(B[i].L, B[i].W, B[i].ind);
    }
    int cur = query(MAXLWH - 1, MAXLWH - 1);
    writeln(VAL[cur]);
    vector<int> path;
    for (; cur != N; cur = PREV[cur]) path.pb(cur);
    writeln(sz(path));
    for (int ind : path) write(ind + 1, "");
    writeln();
    return 0;
}
