...
int N, sqrtN, Q, A[MAXN], MAX1[SQRTN], MAX2[SQRTN], MIN1[SQRTN], MIN2[SQRTN], FLAVOR[MAXN], LOIND[MAXN], HIIND[MAXN];
...
//    freopen("out.txt", "w", stdout);
    ...
    for (int i = 1; i <= N; i++) {
        read(A[i]);
        FLAVOR[i] = (i + sqrtN - 1) / sqrtN;
        MIN(LOIND[FLAVOR[i]], i);
        MAX(HIIND[FLAVOR[i]], i);
    }
    for (int i = 1; i <= sqrtN; i++) {
        MAX1[i] = MAX2[i] = -INT_INF;
        MIN1[i] = MIN2[i] = INT_INF;
        for (int j = LOIND[i]; j <= HIIND[i]; j++) {
            if (A[j] < MIN1[i]) {
                MIN2[i] = MIN1[i];
                MIN1[i] = A[j];
            } else if (A[j] < MIN2[i]) {
