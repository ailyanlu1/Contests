...
int N, Q, dist[MAXNS][MAXNS];
ll preHash[MAXNS][MAXNS], sufHash[MAXNS][MAXNS];
string S[MAXNS];
vector<int> adj[MAXNS];
bool vis[MAXNS];
...
//    freopen("out.txt", "w", stdout);
    ...
    FOR(i, N) {
        read(S[i]);
        preHash[i][0] = 0;
        FOR(j, sz(S[i])) preHash[i][j + 1] = (preHash[i][j] * BASE + S[i][j] - 'A' + 1) % MOD;
        sufHash[i][N + 1] = 0;
        REV(j, sz(S[i])) sufHash[i][j + 1] = (sufHash[i][j + 2] * BASE + S[i][j] - 'A' + 1) % MOD;
    }
    FOR(i, N) {
        FOR(j, i) {
            int a = i, b = j;
            if (sz(S[i]) > sz(S[j])) swap(a, b);
            if (sz(S[a]) == sz(S[b])) {
                int lo = 1, hi = sz(S[a]) + 1, mid;
                while (lo < hi) {
                    mid = lo + (hi - lo) / 2;
                    if (preHash[a][mid] == preHash[b][mid]) lo = mid + 1;
                    else hi = mid;
                }
                assert(lo != sz(S[a]) + 1);
                if (sufHash[a][lo + 1] == sufHash[b][lo + 1]) {
                    adj[a].pb(b);
                    adj[b].pb(a);
                }
            } else if (sz(S[a]) + 1 == sz(S[b])) {
                int lo = 1, hi = sz(S[a]) + 1, mid;
                while (lo < hi) {
                    mid = lo + (hi - lo) / 2;
                    if (preHash[a][mid] == preHash[b][mid]) lo = mid + 1;
                ...
                if (sufHash[a][lo] == sufHash[b][lo + 1]) {
                    adj[a].pb(b);
                    adj[b].pb(a);
                }
            }
    ...
    FOR(i, Q) {
        read(a, b);
        a--; b--;
        if (dist[a][b] == INT_INF) writeln(-1);
        else writeln(dist[a][b]);
    }
    return 0;
}
