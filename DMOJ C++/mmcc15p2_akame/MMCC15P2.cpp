...
public:
    // in non decreasing order of slope, as sorted by the comparator
    void addLine(T m, T b) {
        while (back >= 2 && (B[back - 2] - B[back - 1]) * (m - M[back - 1]) >= (B[back - 1] - b) * (M[back - 1] - M[back - 2])) {
            M.pop_back();
            B.pop_back();
            back--;
        }
    ...
    // lines must be in non decreasing order of slope, as sorted by the comparator
    void addAll(vector<pair<T, T>> &lines) {
        for (pair<T, T> &p : lines) addLine(p.first, p.second);
    }

    // in non decreasing order of x, regardless of comparator
    T getMax(T x) {
...
int N, M, ans[MAXNM];
vector<pdd> LO[4 * MAXNM], HI[4 * MAXNM];
pair<pdd, int> queries[MAXNM];
ConvexHullTrick<double, greater<double>> lower[4 * MAXNM];
ConvexHullTrick<double, less<double>> upper[4 * MAXNM];
...
void build(int cur, int cL, int cR) {
    if (cL == cR) {
        ...
        LO[cur].pb(p);
        HI[cur].pb(p);
        lower[cur].addAll(LO[cur]);
        upper[cur].addAll(HI[cur]);
        return;
    }
    int m = cL + (cR - cL) / 2;
    build(cur * 2, cL, m);
    build(cur * 2 + 1, m + 1, cR);
    merge(all(LO[cur * 2]), all(LO[cur * 2 + 1]), back_inserter(LO[cur]), greater<pdd>());
    merge(all(HI[cur * 2]), all(HI[cur * 2 + 1]), back_inserter(HI[cur]), less<pdd>());
    lower[cur].addAll(LO[cur]);
    upper[cur].addAll(HI[cur]);
}

int query(int cur, int cL, int cR, bool lo, pdd p) {
    if (cL == cR) {
        if (lo && lower[cur].getMax(p.f) <= p.s + EPS) return cL;
        else if (!lo && upper[cur].getMax(p.f) >= p.s - EPS) return cL;
        else return cL + 1;
    }
    int m = cL + (cR - cL) / 2;
    if (lo && lower[cur * 2].getMax(p.f) <= p.s + EPS) return query(cur * 2, cL, m, lo, p);
    else if (!lo && upper[cur * 2].getMax(p.f) >= p.s - EPS) return query(cur * 2, cL, m, lo, p);
    else return query(cur * 2 + 1, m + 1, cR, lo, p);
}
...
//    freopen("out.txt", "w", stdout);
    ...
    build(1, 0, N - 1);
    FOR(i, M) {
        read(queries[i].f.f, queries[i].f.s);
        queries[i].s = i;
    }
    sort(queries, queries + M);
    FOR(i, M) ans[queries[i].s] = max(query(1, 0, N - 1, true, queries[i].f), query(1, 0, N - 1, false, queries[i].f));
    FOR(i, M) writeln(ans[i] == N ? -1 : ans[i] + 1);
    return 0;
}
