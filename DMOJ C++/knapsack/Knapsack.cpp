...
//    freopen("out.txt", "w", stdout);
    ...
    read(N, M);
    ll ni, pi, vi, sum, mult, ci, fi, ans = -LL_INF;;
    FOR(i, N) {
        read(ni, vi, pi);
        for (MIN(ni, MAXNC / vi), sum = 0, mult = 1; sum + mult < ni; sum += mult, mult *= 2) items.eb(mult * vi, mult * pi);
        items.eb((ni - sum) * vi, (ni - sum) * pi);
    }
    dp.fill(0);
    for (pll i : items) Rev(j, MAXNC - 1, i.f - 1) MAX(dp[j], dp[j - i.f] + i.s);
    FOR(i, M) {
        read(ci, fi);
        MAX(ans, dp[ci] - fi);
    }
