...
int N, M, MN[MAXN * 2], GCD[MAXN * 2], CNT[MAXN * 2];

void merge(int i, int j, int k) {
    MN[i] = min(MN[j], MN[k]);
    int gcd = __gcd(GCD[j], GCD[k]);
    CNT[i] = (gcd == GCD[j]) * CNT[j] + (gcd == GCD[k]) * CNT[k];
    GCD[i] = gcd;
}
...
//    freopen("out.txt", "w", stdout);
    ...
    Rev(i, N - 1, 0) merge(i, i << 1, i << 1 | 1);
    char op;
    int a, b;
    FOR(m, M) {
        read(op, a, b);
        if (op == 'C') {
            for (a += N - 1, MN[a] = GCD[a] = b, CNT[a] = 1; a >>= 1; merge(a, a << 1, a << 1 | 1));
        } else {
            MN[0] = INT_INF;
            GCD[0] = CNT[0] = 0;
