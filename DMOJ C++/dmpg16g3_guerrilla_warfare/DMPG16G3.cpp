...
int N, Q, UF[MAXN], PRE[MAXN], POST[MAXN], PAR[MAXN], DEP[MAXN], SZ[MAXN], cur = 0;
ll BIT[MAXN * 2], ans1[MAXN], ans2[MAXN];
vector<int> adj[MAXN];
...
void dfs2(int v, int prev) {
    if (prev == -1) ans2[v] = ans1[v];
    else ans2[v] = ans1[v] + ans2[prev] - ans1[v] + N - 2 * SZ[v];
    update(PRE[v], ans2[v]);
    update(PRE[v] + 1, -ans2[v]);
    for (int w : adj[v]) if (w != prev) dfs2(w, v);
}
    ...
    FOR(i, Q) {
        ...
        if (t == 1) {
            ...
            while ((a = find(a)) != (b = find(b))) {
                ...
                update(PRE[0], -SZ[a]);
                update(PRE[a], 2 * SZ[a] - N);
                update(POST[a], N - 2 * SZ[a]);
                UF[a] = find(PAR[a]);
                a = PAR[a];
            }
        } else {
