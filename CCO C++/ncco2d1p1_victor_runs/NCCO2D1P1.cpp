...
int N, T, A[MAXN], dp[2][MAXN][MAXN][2];
...
//    freopen("out.txt", "w", stdout);
    ...
    int ans = 0;
    FOR(j, N) For(k, j, N) FOR(m, 2) dp[0][j][k][m] = 0;
    for (int i = 1; i < N; MAX(ans, i * T - dp[i % 2][lb][lb][0]), i++) FOR(j, N) For(k, j, N) FOR(m, 2)
        dp[i % 2][j][k][m] = min(j == 0 ? INT_INF : dp[1 - i % 2][j - 1][k][0] + i * (A[m ? k : j] - A[j - 1]), k == N - 1 ? INT_INF : dp[1 - i % 2][j][k + 1][1] + i * (A[k + 1] - A[m ? k : j]));
    writeln(ans);
    return 0;
}
