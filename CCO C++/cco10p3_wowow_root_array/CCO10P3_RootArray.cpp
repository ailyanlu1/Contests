...
int N, rating[MAXN];
OrderedRootArray<4, pii, OrderedRootArray<3, pii, OrderedSqrtArray<pii, greater<pii>>, greater<pii>>, greater<pii>> arr(3);
//OrderedRootArray<3, pii, OrderedSqrtArray<pii, greater<pii>>, greater<pii>> arr(3);
...
//    freopen("out.txt", "w", stdout);
    ...
    FOR(i, N) {
        ...
        if (op == 'N') {
            ...
            cin >> x >> r;
            arr.erase({rating[x], x});
            arr.insert({r, x});
            rating[x] = r;
        } else if (op == 'Q') {
            cin >> x;
            cout << arr.at(x - 1).second << nl;
        } else {
            i--;
        }
    }
