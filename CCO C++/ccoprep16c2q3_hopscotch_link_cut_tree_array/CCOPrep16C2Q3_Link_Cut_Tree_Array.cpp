...
void init() {
    FOR(i, N + 1) {
        LCT[i][0] = LCT[i][1] = LCT[i][2] = -1;
        LCT[i][3] = 1;
    }
}

bool isRoot(int x) {
    return LCT[x][2] == -1 || (x != LCT[LCT[x][2]][0] && x != LCT[LCT[x][2]][1]);
}

int getSize(int x) {
    return x == -1 ? 0 : LCT[x][3];
}

void update(int x) {
    if (x != -1) LCT[x][3] = 1 + getSize(LCT[x][0]) + getSize(LCT[x][1]);
}

void connect(int ch, int par, bool hasCh, bool isL) {
    if (ch != -1) LCT[ch][2] = par;
    if (hasCh) LCT[par][!isL] = ch;
}
...
void rotate(int x) {
    ...
    bool isL = x == LCT[p][0];
    connect(LCT[x][isL], p, true, isL);
    connect(p, x, true, !isL);
    connect(x, g, !isRootP, isRootP ? false : p == LCT[g][0]);
    update(p);
}
...
void splay(int x) {
    while (!isRoot(x)) {
        int p = LCT[x][2];
        int g = LCT[p][2];
        if (!isRoot(p)) rotate((x == LCT[p][0]) == (p == LCT[g][0]) ? p : x);
        rotate(x);
    }
    update(x);
}
