// https://www.hackerearth.com/practice/math/combinatorics/inclusion-exclusion/practice-problems/algorithm/important-killers-5990f168/
import java.io.*;
import java.math.*;
import java.util.*;
 
public class JulyEasy18P3 {
    ...
    public static void run(int testCaseNum) throws Exception {
        if (testCaseNum == 1) sieve(BOUND);
        int P = in.nextInt(), X = in.nextInt() - 1;
        int cnt = 0;
        ArrayList<Long> vals = new ArrayList<Long>();
        while (primes.get(cnt) <= P) cnt++;
        for (int i = 0; i < (1 << cnt); i++) {
            long val = 1;
            for (int j = 0; j < cnt; j++) if (((i >> j) & 1) == 1) val *= primes.get(j);
            vals.add(val);
        }
        Collections.sort(vals);
        if (X >= vals.size()) out.println(-1);
        else out.println(vals.get(X));
    }
}
