...
public class WeekOfCode37P3 {
    private static WeekOfCode37P3 o = new WeekOfCode37P3();
    ...
    private static int NUM_OF_TEST_CASES; // TODO CHANGE NUMBER OF TEST CASES
    ...
    public static void run(int testCaseNum) throws Exception {
        long freq, len = 0, ans = 0;
        for (int i = 0; i < 26; i++) {
            freq = in.nextLong();
            if (freq == 0) continue;
            if (len >= 2) {
                long add = Math.min(freq, len - 1);
                if (add == len - 1) ans = add;
                else ans = add + Math.min(ans, (len - add - 1) / 2);
            }
            len += freq;
        }
        out.println(ans);
