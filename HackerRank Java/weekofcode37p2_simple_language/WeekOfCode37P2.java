...
public class WeekOfCode37P2 {
    private static WeekOfCode37P2 o = new WeekOfCode37P2();
    ...
    public static void run(int testCaseNum) throws Exception {
        ...
        long cur = 0;
        for (int i = 0; i < N; i++) {
            if (in.next().equals("add")) cur += Math.max(0, in.nextLong());
            else cur = Math.max(cur, in.nextLong());
        }
        out.println(cur);
    }
}
