// https://www.hackerearth.com/practice/algorithms/graphs/depth-first-search/practice-problems/algorithm/tree-and-xor-1-aaedc91e/
#include <bits/stdc++.h>
using namespace std;
...
int N, cur = -1, X[MAXN], pre[MAXN], post[MAXN], preVert[MAXN], CH[MAXN * 120][2], prefix[MAXN], suffix[MAXN], curNode = 0;
vector <int> adj[MAXN], postVert[MAXN];
 
int makeNode(int i = 0) {
    assert(++curNode < MAXN * 120);
    CH[curNode][0] = CH[i][0];
    CH[curNode][1] = CH[i][1];
    return curNode;
}
 
int insert(int cur, int val, int d) {
    if (d == -1) return makeNode(cur);
    int ret = makeNode(cur);
    int v = (val >> d) & 1;
    CH[ret][1 - v] = CH[cur][1 - v];
    CH[ret][v] = insert(CH[cur][v], val, d - 1);
    return ret;
}
 
int getMax(int cur, int val, int d) {
    if (d == -1 || !cur) return 0;
    int v = (val >> d) & 1;
    if (CH[cur][1 - v]) return (1 << d) + getMax(CH[cur][1 - v], val, d - 1);
    return getMax(CH[cur][v], val, d - 1);
}
...
ll dfs2(int v, int curXor, int prev) {
    ll ret = 0;
    for (int w : adj[v]) if (w != prev) ret += dfs2(w, max(curXor, X[v]), v);
    return ret + max(curXor, max(pre[v] == 0 ? 0 : getMax(prefix[pre[v] - 1], X[v], 30), post[v] == N - 1 ? 0 : getMax(suffix[post[v] + 1], X[v], 30)));
}
...
//    freopen("out.txt", "w", stdout);
    ...
    dfs1(0, -1);
    FOR(i, N) {
        prefix[i] = makeNode(i == 0 ? 0 : prefix[i - 1]);
        for (int v : postVert[i]) prefix[i] = insert(prefix[i], X[v], 30);
    }
    REV(i, N - 1) {
        suffix[i] = makeNode(i == N - 1 ? 0 : suffix[i + 1]);
        suffix[i] = insert(suffix[i], X[preVert[i]], 30);
    }
    writeln(dfs2(0, 0, -1));
    return 0;
}
