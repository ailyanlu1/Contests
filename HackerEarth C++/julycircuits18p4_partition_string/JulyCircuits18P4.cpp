// https://www.hackerearth.com/problem/algorithm/partition-string-1-17f6467d/
#include <bits/stdc++.h>
using namespace std;
...
//    freopen("out.txt", "w", stdout);
    ...
    setLength(100005);
    POW[0] = 1;
    For(i, 1, 30) POW[i] = POW[i - 1] * BASE % HMOD;
    read(S, N);
    string Ti;
    FOR(i, N) {
        read(Ti);
        ll tFreq = 0;
        FOR(j, sz(Ti)) (tFreq += POW[Ti[j] - 'a']) %= HMOD;
        H[sz(Ti)].insert(tFreq);
        lens.pb(sz(Ti));
    }
    sort(all(lens));
    ...
    For(i, 1, sz(S) + 1) {
        dp[i] = 0;
        freq[i] = (freq[i - 1] + POW[S[i - 1] - 'a']) % HMOD;
        for (int L : lens) {
            if (L > i) break;
            if (H[L].count((freq[i] - freq[i - L] + HMOD) % HMOD)) (dp[i] += dp[i - L]) %= MOD;
        }
    }
    writeln(dp[sz(S)]);
    return 0;
