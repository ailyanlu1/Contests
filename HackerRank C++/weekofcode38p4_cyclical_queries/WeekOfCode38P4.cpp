...
public:
    ...
    Node (int vertex, ll val, double priority, int type, int size = 1) : vertex(vertex), val(val), priority(priority), type(type), size(size) {}
};
...
ll subtreeVal(Node *x) {
    return x ? x->subtreeVal : 0;
}

void apply(Node *x, ll val) {
    if (x) {
        x->subtreeVal += val;
        x->val += val;
        x->delta += val;
...
void update(Node *x) {
    if (x) {
        x->size = 1;
        x->subtreeVal = x->val;
        x->subtreeVert = x->vertex;
        if (x->left) {
            x->left->parent = x;
            x->size += x->left->size;
            if (x->type == MAXNODE) {
                if (x->subtreeVal < x->left->subtreeVal) {
                    x->subtreeVal = x->left->subtreeVal;
                    x->subtreeVert = x->left->subtreeVert;
                } else if (x->subtreeVal == x->left->subtreeVal && x->subtreeVert < x->left->subtreeVert) {
                    x->subtreeVal = x->left->subtreeVal;
                    x->subtreeVert = x->left->subtreeVert;
                }
            } else x->subtreeVal += x->left->subtreeVal;
        }
        if (x->right) {
            x->right->parent = x;
            x->size += x->right->size;
            if (x->type == MAXNODE) {
                if (x->subtreeVal < x->right->subtreeVal) {
                    x->subtreeVal = x->right->subtreeVal;
                    x->subtreeVert = x->right->subtreeVert;
                } else if (x->subtreeVal == x->right->subtreeVal && x->subtreeVert < x->right->subtreeVert) {
                    x->subtreeVal = x->right->subtreeVal;
                    x->subtreeVert = x->right->subtreeVert;
                }
            } else x->subtreeVal += x->right->subtreeVal;
        }
    }
}
...
void propagate(Node *x) {
    if (x && x->delta) {
        if (x->left) {
            x->left->subtreeVal += x->delta;
            x->left->val += x->delta;
            x->left->delta += x->delta;
        }
        if (x->right) {
            x->right->subtreeVal += x->delta;
            x->right->val += x->delta;
...
void init() {
    ...
    post.reserve(N + M);
    dist.reserve(N);
    Node *dummyMax = nullptr, *dummySum = nullptr;
    ll curDist = 0;
    FOR(i, N) {
        pre.pb(new Node(i, curDist, dis(rng), MAXNODE));
        post.pb(new Node(i, curDist, dis(rng), MAXNODE));
        merge(dummyMax, pre.back(), post.back());
        curDist += W[i];
    }
    For(i, N, N + M) {
        pre.pb(new Node(i, 0, dis(rng), MAXNODE));
        post.pb(new Node(i, 0, dis(rng), MAXNODE));
        merge(dummyMax, pre.back(), post.back());  
    }
    FOR(i, N) {
        dist.pb(new Node(i, W[i], dis(rng), SUMNODE));
        merge(dummySum, dummySum, dist.back());
        if (i < N - 1) addEdge(i, i + 1);
    }
}

Node *makeRoot(int v) {
    Node *l1 = nullptr, *r1 = nullptr, *l2 = nullptr, *mid2 = nullptr, *r2 = nullptr;
    split(root(dist[v]), l1, r1, index(dist[v]));
    split(root(pre[v]), l2, mid2, index(pre[v]));
    split(mid2, mid2, r2, index(post[v]) + 1);
    apply(mid2, -subtreeVal(l1));
    merge(l2, l2, r2);
    apply(l2, subtreeVal(r1));
    Node *treeRootl2 = min(l2);
    if (treeRootl2) {
        int prev = (treeRootl2->vertex + N - 1) % N;
        Node *r3 = nullptr;
        split(mid2, mid2, r3, index(pre[prev]) + 1);
        merge(mid2, mid2, l2);
        merge(mid2, mid2, r3);
    }
    merge(r1, r1, l1);
...
//    freopen("out.txt", "w", stdout);
    ...
    FOR(i, M) {
        ...
        if (t == 1) {
            Node *T = makeRoot(x);
            read(w);
            apply(pre[curNode], T->subtreeVal + w);
            addEdge(T->subtreeVert, curNode++);
        } else if (t == 2) {
            makeRoot(x);
            read(w);
            apply(pre[curNode], w);
            ...
            Node *T = makeRoot(x);
            cutParent(T->subtreeVert);
        } else {
            Node *T = makeRoot(x);
            writeln(T->subtreeVal);
        }
    }
    return 0;
}
