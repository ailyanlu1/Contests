...
int N;
ll X[MAXN], Y[MAXN], Z[MAXN], H[MAXN], C[MAXN], L[MAXN], G[MAXN];
...
int main() {
    ...
    For(i, 1, N + 1) {
        ...
        C[i] = Y[i] ^ G[i - 1];
        L[i] = Z[i] ^ G[i - 1];
        maxJ = {LONG_LONG_MIN, -1};
        For(j, 1, i - L[i] + 1) MAX(maxJ, mp((ll) round(abs(sqrt(H[j] * H[j] + C[j] * C[j])) * abs(sqrt(H[i] * H[i] + C[i] * C[i])) * sin(atan2(C[i], H[i]) - atan2(C[j], H[j]))), j));
        G[i] = (G[i - 1] + (maxJ.f % MOD)) % MOD;
        if (G[i] < 0) G[i] += MOD;
    }
    printf("%lld\n", G[N]);
    return 0;
