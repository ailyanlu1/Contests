...
struct Node {
    int l = -1, r = -1, p = -1, lo = INT_INF, hi = -1;
};

ll dfs(vector<Node> &T, int cur) {
    if (T[cur].l == -1) {
        T[cur].lo = T[cur].hi = T[cur].r;
        return 0;
    }
    ll lRet = dfs(T, T[cur].l);
    ll rRet = dfs(T, T[cur].r);
    T[cur].lo = min(T[T[cur].l].lo, T[T[cur].r].lo);
    T[cur].hi = max(T[T[cur].l].hi, T[T[cur].r].hi);
    if (lRet == -1 || rRet == -1) return -1;
    if (T[T[cur].l].lo > T[T[cur].r].hi) return lRet + rRet + 1;
    else if (T[T[cur].l].hi > T[T[cur].r].lo) return -1;
    return lRet + rRet;
}
...
void solve() {
    ...
    FOR(i, N) {
        ...
        if (T[i].l != -1) {
            T[i].l--;
            T[i].r--;
            T[T[i].l].p = T[T[i].r].p = i;
        }
    }
    int root = -1;
    FOR(i, N) if (T[i].p == -1) root = i;
    writeln(dfs(T, root));
}
