...
void solve() {
    ...
    read(N, M);
    if (N >= M) {
        vector<vector<int>> T(2 * N, vector<int>(M));
        FOR(i, N) {
            FOR(j, M) {
                read(T[N + i][j]);
                T[N + i][j]--;
        ...
        Rev(i, N - 1, 0) T[i] = mult(T[i << 1], T[i << 1 | 1]);
        read(Q);
        int l, r;
        FOR(i, Q) {
            read(l, r);
            ...
            for (l += N - 1, r += N - 1; l <= r; l >>= 1, r >>= 1) {
                if (l & 1) ql = mult(ql, T[l++]);
                if (!(r & 1)) qr = mult(T[r--], qr);
            }
            vector<int> res = mult(ql, qr);
            ll ans = 0;
            FOR(i, M) ans += ((ll) (i + 1)) * ((ll) (res[i] + 1));
            writeln(ans);
        }
    } else {
        ...
        FOR(i, N) {
            ...
            For(j, i, N) {
                cur = mult(cur, P[j]);
                FOR(k, M) ans[i][j] += ((ll) (k + 1)) * ((ll) (cur[k] + 1));
            }
        }
        read(Q);
        int l, r;
