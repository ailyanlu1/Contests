...
//    freopen("out.txt", "w", stdout);
    ...
    read(N);
    FOR(i, N) {
        read(V[i].s.f, V[i].s.s);
        V[i].f = i;
    }
    pll cur;
    do {
        random_shuffle(V, V + N, rng);
        cur = {0, 0};
        FOR(i, N) {
            pll pos = {cur.f + V[i].s.f, cur.s + V[i].s.s};
            pll neg = {cur.f - V[i].s.f, cur.s - V[i].s.s};
            if (pos.f * pos.f + pos.s * pos.s < neg.f * neg.f + neg.s * neg.s) {
                ans[V[i].f] = 1;
                cur = pos;
            } else {
                ans[V[i].f] = -1;
    ...
    } while (cur.f * cur.f + cur.s * cur.s > 1500000LL * 1500000LL);
    FOR(i, N) {
        if (i > 0) write(' ');
        write(ans[i]);
    }
