...
int N, ans[MAXN], size[MAXN], depth[MAXN], isHeavy[MAXN], curBest = -1, CNT[MAXN], T[MAXN * 2];
vector<int> adj[MAXN];

int merge(int a, int b) {
    if (CNT[a] == CNT[b]) return min(a, b);
    else if (CNT[a] > CNT[b]) return a;
    else return b;
}
...
void update(int i, int v) {
    CNT[i] += v;
    for (i += N; i >>= 1;) T[i] = merge(T[i << 1], T[i << 1 | 1]);
}

void preDfs(int v, int prev, int d) {
    size[v] = 1;
    depth[v] = d;
    for (int w : adj[v]) {
        if (w == prev) continue;
...
//    freopen("out.txt", "w", stdout);
    ...
    FOR(i, N) CNT[i] = isHeavy[i] = 0;
    preDfs(0, -1, 0);
    init();
    dfs(0, -1, 0);
    FOR(i, N) writeln(ans[i]);
    return 0;
}
