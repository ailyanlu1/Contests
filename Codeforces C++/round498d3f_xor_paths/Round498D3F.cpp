...
int N, M;
ll K, A[MAXNM][MAXNM], ans = 0;
umap<ll, ll> X[MAXNM];

void down(int i, int j, ll x) {
    x ^= A[i][j];
    if (i + j == N - 1) X[j][x]++;
    if (i + 1 + j <= N - 1 && i + 1 < N) down(i + 1, j, x);
    if (j + 1 + i <= N - 1 && j + 1 < M) down(i, j + 1, x);
}

void up(int i, int j, ll x) {
    x ^= A[i][j];
    if (i + j == N - 1) ans += X[j][K ^ x ^ A[i][j]];
    if (i - 1 + j >= N - 1 && i - 1 >= 0) up(i - 1, j, x);
    if (j - 1 + i >= N - 1 && j - 1 >= 0) up(i, j - 1, x);
}
