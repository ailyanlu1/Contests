// https://codeforces.com/contest/1017/problem/H
#include <bits/stdc++.h>
using namespace std;
...
struct Query {
    ...
    bool operator < (const Query &q) {
        return bl == q.bl ? r < q.r : bl < q.bl;
    }
};

int N, Q, A[MAXNMQ], cnt[MAXNMQ * 2], cnt2[MAXNMQ * 2];
umap<int, vector<ll>> mult;
ll M, inv[MAXNMQ * 2], ans[MAXNMQ], curAns;
vector<Query> q[MAXNMQ];

ll powMod(ll base, ll pow) {
    ll ret = 1;
    for (base %= MOD; pow > 0; pow /= 2) {
        if (pow % 2 == 1) ret = ret * base % MOD;
        base = base * base % MOD;
    }
    return ret;
}
...
void add(int val, int k) {
    curAns = curAns * (k + cnt[val] - (++cnt2[val]) + 1) % MOD;
}

void rem(int val, int k) {
    curAns = curAns * inv[k + cnt[val] - (cnt2[val]--) + 1] % MOD;
}
...
//    freopen("out.txt", "w", stdout);
    ...
    FOR(i, Q) {
        ...
        --l; --r;
        q[k].pb({l, r, l / BSZ, i});
        if (!mult.count(k)) mult[k] = vector<ll>();
    }
    for (auto &&p : mult) {
        p.s.pb(1);
        For(i, 1, N + 1) p.s.pb(p.s.back() * (p.f * M % MOD + i) % MOD);
        FOR(i, MAXNMQ * 2) cnt2[i] = 0;
        sort(all(q[p.f]));
        curAns = 1;
        int l = q[p.f][0].l;
        int r = l - 1;
        for (auto &&q : q[p.f]) {
            while (l < q.l) rem(A[l++], p.f);
            while (l > q.l) add(A[--l], p.f);
            while (r < q.r) add(A[++r], p.f);
            while (r > q.r) rem(A[r--], p.f);
            ans[q.ind] = curAns * p.s[N - (q.r - q.l + 1)] % MOD;
        }
    }
    FOR(i, Q) writeln(ans[i]);
    return 0;
}
