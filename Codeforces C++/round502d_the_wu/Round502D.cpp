...
int N, M, Q, W[MAXN], cnt[1 << MAXN], WU[1 << MAXN], ans[1 << MAXN][MAXW];
...
//    freopen("out.txt", "w", stdout);
    ...
    FOR(i, 1 << N) FOR(j, N) WU[i] += (((i >> j) & 1) ^ 1) * W[j];
    FOR(i, 1 << N) FOR(j, 1 << N) if (WU[i ^ j] < MAXW) ans[i][WU[i ^ j]] += cnt[j];
    FOR(i, 1 << N) For(j, 1, MAXW) ans[i][j] += ans[i][j - 1];
    int k;
    FOR(i, Q) {
        read(S, k);
        int v = 0;
