...
//    freopen("out.txt", "w", stdout);
    ...
    while (true) {
        int startInd = -1;
        FOR(i, 2 * N) {
            if (grid[ord[i].f][ord[i].s] != 0 && grid[ord[i].f][ord[i].s] == grid[ord[i].f ^ 1][ord[i].s]) {
                moves.pb({grid[ord[i].f][ord[i].s], {ord[i].f ^ 1, ord[i].s}});
                grid[ord[i].f][ord[i].s] = 0;
                done++;
            }
            if (grid[ord[i].f][ord[i].s] == 0) startInd = i;
        }
        if (startInd == -1 || done == K) break;
        FOR(h, 2 * N - 1) {
            int i = (startInd + h) % (2 * N);
            if (grid[ord[(i + 1) % (2 * N)].f][ord[(i + 1) % (2 * N)].s] == 0) continue;
            grid[ord[i].f][ord[i].s] = grid[ord[(i + 1) % (2 * N)].f][ord[(i + 1) % (2 * N)].s];
            moves.pb({grid[ord[i].f][ord[i].s], {ord[i].f, ord[i].s}});
            grid[ord[(i + 1) % (2 * N)].f][ord[(i + 1) % (2 * N)].s] = 0;
        }
    }
    if (done != K) writeln(-1);
    else {
        writeln(sz(moves));
        for (pair<int, pii> x : moves) writeln(x.f, x.s.f + 1, x.s.s + 1);
    }
    return 0;
}
