...
template <typename It, typename Comparator> int count_inversions(It src_st, It src_en, It dst_st, It dst_en, Comparator cmp) {
    int n = src_en - src_st;
    if (n <= 1) return 0;
    int mid = (n - 1) / 2;
    int ret = 0;
...
template <typename It, typename Comparator> int count_inversions(It st, It en, Comparator cmp) {
...
int N, A[MAXN * 2], B[MAXN * 2], firstInd[MAXN], lastInd[MAXN];

bool cmpFirst(int i, int j) {
    return firstInd[i] < firstInd[j];
}

bool cmpLast(int i, int j) {
    return lastInd[i] < lastInd[j];
}
...
//    freopen("out.txt", "w", stdout);
    ...
    writeln(min(count_inversions(A, A + (2 * N), cmpFirst), count_inversions(B, B + (2 * N), cmpLast)));
    return 0;
}
