...
string S;
int origFreq[10], freq[10];
ll fact[20];

ll calc(int len) {
    ll ret = fact[len - freq[0]];
    For(i, 1, 10) ret /= fact[freq[i]];
    ll zeros = fact[len - 1] / fact[freq[0]] / fact[len - 1 - freq[0]];
    ret *= zeros;
    return ret;
}

ll solve(int i, int len) {
    if (i == 10) return calc(len);
    ll ret = 0;
    For(j, 1, origFreq[i] + 1) {
        freq[i] = j;
        ret += solve(i + 1, len + j);
    }
    if (origFreq[i] == 0) ret += solve(i + 1, len);
    return ret;
}
...
//    freopen("out.txt", "w", stdout);
    ...
    FOR(i, 10) origFreq[i] = freq[i] = 0;
    FOR(i, sz(S)) origFreq[S[i] - '0'] = ++freq[S[i] - '0'];
    writeln(solve(0, 0));
    return 0;
}
