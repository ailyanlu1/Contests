...
#define MAXNODES 15000005

int N, Q, L[MAXNODES], R[MAXNODES], VAL[MAXNODES], LZ[MAXNODES], root = 1, curNode = 1;

void propagate(int cur, int cL, int cR) {
    if (LZ[cur] != -1) {
        int m = cL + (cR - cL) / 2;
        VAL[L[cur]] = LZ[cur] * (m - cL + 1);
        LZ[L[cur]] = LZ[cur];
        VAL[R[cur]] = LZ[cur] * (cR - m);
        LZ[R[cur]] = LZ[cur];
        LZ[cur] = -1;
    }
}
...
void update(int cur, int cL, int cR, int l, int r, int val) {
    ...
    propagate(cur, cL, cR);
    if (m >= l) update(L[cur], cL, m, l, r, val);
    if (m < r) update(R[cur], m + 1, cR, l, r, val);
