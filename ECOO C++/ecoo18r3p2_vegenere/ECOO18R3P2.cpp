...
mt19937 gen(seq);
const ll BASE = uniform_int_distribution<ll>(0, HMOD - 1)(gen);
...
uset<ll> hs[MAXS];
ll HASH[MAXS], dp[MAXS], POW[MAXS];

ll powMod(ll base, ll pow, ll mod) {
    ll x = 1, y = base % mod;
    for (; pow > 0; pow /= 2) {
        if (pow % 2 == 1) x = x * y % mod;
        y = y * y % mod;
    }
    return x;
}
...
bool dfs(int i, int j, Node *cur) {
    if (j == sz(S)) return cur->end;
    if (cur->end && dfs(i, j, root)) return true;
    char c = 'A' + (S[j] - 'A' - (dict[i][j % sz(dict[i])] - 'A' + 1) + 26) % 26;
    return cur->child.count(c) ? dfs(i, j + 1, cur->child[c]) : false;
}
...
void solve(int t) {
    ...
    ans.clear();
    lens.clear();
    FOR(i, MAXS) hs[i].clear();
    read(N);
    FOR(i, N) {
        read(dict[i]);
        insert(dict[i]);
        ll h = 0;
        FOR(j, sz(dict[i])) h = (h + POW[j] * (dict[i][j] - 'A')) % HMOD;
        hs[sz(dict[i])].insert(h);
        lens.pb(sz(dict[i]));
    }
    sort(all(lens));
    ...
    FOR(i, N) {
        if (dfs(i, 0, root)) {
            string s = "";
            FOR(j, sz(S)) s.pb('A' + (S[j] - 'A' - (dict[i][j % sz(dict[i])] - 'A' + 1) + 26) % 26);
            HASH[0] = 0;
            dp[0] = 0;
            FOR(j, sz(s)) {
                dp[j + 1] = LL_INF;
                HASH[j + 1] = (HASH[j] + POW[j] * (s[j] - 'A')) % HMOD;
                for (int L : lens) {
                    if (L > j + 1) break;
                    if (hs[L].count(divMod((HASH[j + 1] - HASH[j + 1 - L] + HMOD) % HMOD, POW[j + 1 - L], HMOD))) MIN(dp[j + 1], dp[j + 1 - L] + (ll) L * (ll) L);
                }
            }
            ans.eb(s, dp[sz(s)]);
        }
    ...
    sort(all(ans));
    writeln(ans[0].f.substr(0, 10), ans[0].s);
}

void preProcess() {
    POW[0] = 1;
    For(i, 1, MAXS) POW[i] = POW[i - 1] * BASE % HMOD;
}
