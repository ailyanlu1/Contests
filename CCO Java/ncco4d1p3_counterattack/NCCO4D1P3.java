...
public class NCCO4D1P3 {
    private static NCCO4D1P3 o = new NCCO4D1P3();
    ...
    public static void run(int testCaseNum) throws Exception {
        ...
        while (!pq.isEmpty()) {
            Pair v = pq.poll();
            for (Pair e : adj[v.second]) {
                if (dist[e.second][0] > v.first + e.first) {
                    dist[e.second][1] = dist[e.second][0];
                    dist[e.second][0] = v.first + e.first;
                    pq.offer(new Pair(v.first + e.first, e.second));
                } else if (dist[e.second][0] != v.first + e.first && dist[e.second][1] > v.first + e.first) {
                    dist[e.second][1] = v.first + e.first;
                    pq.offer(new Pair(v.first + e.first, e.second));
                }
            }
        }
