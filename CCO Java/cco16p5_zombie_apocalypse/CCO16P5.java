...
public class CCO16P5 {
	private static CCO16P5 o = new CCO16P5();
	public class Reader {
	...
	public static class Position implements Comparable<Position> {
		...
		public Position(long pos, int ind, int type) {
			this.pos = pos;
			this.ind = ind;
			this.type = type;
		}
	...
	private static long getArea(int q) {
		Position[] ver = new Position[2 * K];
		Position[] hor = new Position[2 * K];
		boolean[] use = new boolean[K];

		for (int i = 0; i < K; i++) {
			int loR = Math.max(1, (int) p[i].y() - q);
			int hiR = Math.min(N, (int) p[i].y() + q);
			int loC = Math.max(1, (int) p[i].x() - q);
			int hiC = Math.min(M, (int) p[i].x() + q);
			hor[i << 1] = new Position(loC, i, 1);
			hor[i << 1 | 1] = new Position(hiC + 1, i, -1);
			ver[i << 1] = new Position(loR, i, 1);
			ver[i << 1 | 1] = new Position(hiR + 1, i, -1);
		}
		...
		for (int i = 0; i < 2 * K; i++) {
			...
			for (int j = 0; j < 2 * K; j++) {
				...
				verPrev = ver[j].pos;
				if (use[ver[j].ind]) curr += ver[j].type;
