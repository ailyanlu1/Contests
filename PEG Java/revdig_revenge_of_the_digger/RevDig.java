package revdig_revenge_of_the_digger;
...
public class RevDig {
	private static RevDig o = new RevDig();
	public class Reader {
	...
	public static void main(String[] args) throws IOException {
		int M = in.nextInt();
		int N = in.nextInt();
		FlowNetwork G = o.new FlowNetwork(M * 2);
		int V = 0;
		for (int i = 0; i < M; i++) {
			int f = in.nextInt();
			if (f == 0) {
			...
			G.addEdge(o.new FlowEdge(in(i), out(i), f));
		}
		for (int i = 0; i < N; i++) {
			G.addEdge(o.new FlowEdge(out(in.nextInt() - 1), in(in.nextInt() - 1), Integer.MAX_VALUE));
		}
		EdmondsKarpMaxFlow mf = o.new EdmondsKarpMaxFlow(G, in(0), out(V));
		out.println((int) mf.value());
		out.close();
	}
	
	private static int in(int v) {
		return v * 2;
	}
	
	private static int out(int v) {
		return v * 2 + 1;
	}
}
