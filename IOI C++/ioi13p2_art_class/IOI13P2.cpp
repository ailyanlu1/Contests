...
int T, H, W, R[MAXHW][MAXHW], G[MAXHW][MAXHW], B[MAXHW][MAXHW];

int style(int H, int W, int R[500][500], int G[500][500], int B[500][500]) {
    int val = 0;
    FOR(i, H) FOR(j, W - 1) val += abs(R[i][j] - R[i][j + 1]) + abs(G[i][j] - G[i][j + 1]) + abs(B[i][j] - B[i][j + 1]);
    val /= H * (W - 1);
    if (val <= 8) return 4;
    else if (val <= 25) return 1;
    else if (val <= 53) return 2;
    else return 3;
}
