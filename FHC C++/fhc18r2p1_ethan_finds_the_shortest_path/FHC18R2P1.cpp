...
ll check() {
    ...
    while (true) {
        if (i == N) return d;
        pii best = {INT_INF, INT_INF};
        for (pii e : adj[i]) {
            if (e.s < best.s) best = e;
            else if (e.s == best.s) assert(false);
        }
        if (best.f == INT_INF) assert(false);
        d += best.s;
        i = best.f;
    }
    assert(false);
...
void solve(int t) {
    ...
    if (K <= 2 || N == 2) {
        ...
        writeln(dist - K);
        writeln(sz(ans));
        for (pair<pii, int> e : ans) writeln(e.f.f, e.f.s, e.s);
    }
}

int main() {
    freopen("ethan_finds_the_shortest_path.txt", "r", stdin);
    freopen("ethan_finds_the_shortest_path_out.txt", "w", stdout);
