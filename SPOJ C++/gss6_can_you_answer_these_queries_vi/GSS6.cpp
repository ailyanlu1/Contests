...
int N, M, L[MAXNODES], R[MAXNODES], VAL[MAXNODES], MX[MAXNODES], SUM[MAXNODES], PRE[MAXNODES], SUF[MAXNODES], MAXSUM[MAXNODES], SZ[MAXNODES], root = 0, curNode = 0;
double PRI[MAXNODES];
...
//    freopen("out.txt", "w", stdout);
    ...
    FOR(i, M) {
        ...
        if (op == 'I') {
            ...
            read(p, x);
            split(root, l, mid, p - 1);
            split(mid, mid, r, x - p + 1);
            if (max(mid) < 0) writeln(max(mid));
            else writeln(maxSum(mid));
            merge(root, l, mid);
            merge(root, root, r);
        } else {
            i--;
