...
void solve() {
    ...
    FOR(i, M) {
        cin >> a >> b >> c >> d;
        if (b < c) {
            ans = st->maxSufSum(a, b) + rsq(b + 1, c - 1) + st->maxPreSum(c, d);
        } else {
            ans = st->maxSufSum(a, c - 1) + st->maxPreSum(c, d);
            MAX(ans, st->maxSufSum(c, b) + st->maxPreSum(b + 1, d));
            MAX(ans, st->maxSum(c, b));
        }
        cout << ans << nl;
    }
}
