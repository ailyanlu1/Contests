...
public class DS4_Cbrt_Array {
    ...
    public static class Sqrt {
        ...
        public ArrayList<Integer> prefixSZ;
        public int n;
        ...
        public Sqrt(ArrayList<Integer> b) {
            n = b.size();
            a = new ArrayList<ArrayList<Integer>>();
            prefixSZ = new ArrayList<Integer>();
        ...
        public void removeLast() {
            ...
            int lastSZ = a.get(lastInd).size();
            a.get(lastInd).remove(lastSZ - 1);
            if (a.get(lastInd).isEmpty()) {
                a.remove(lastInd);
                prefixSZ.remove(lastInd);
            }
        }
        
        public int getLast() {
            int lastInd = a.size() - 1;
            return a.get(lastInd).get(a.get(lastInd).size() - 1);
        }
    ...
    public static ArrayList<Sqrt> a;
    public static ArrayList<Integer> prefixSZ;
    public static int n;
    
    public static void init(ArrayList<Integer> b) {
        n = N;
        a = new ArrayList<Sqrt>();
        prefixSZ = new ArrayList<Integer>();
        int cbrtnsq = ((int) (Math.cbrt(n) * Math.cbrt(n))) * FACTOR;
        for (int i = 0; i < N; i += cbrtnsq) {
            a.add(new Sqrt(new ArrayList<Integer>(b.subList(i, Math.min(i + cbrtnsq, n)))));
            prefixSZ.add(i);
        }
    }
    ...
    public static void insert(int val) {
        ...
        while (lo < hi) {
            mid = lo + (hi - lo) / 2;
            if (val < a.get(mid).getLast()) hi = mid;
            else lo = mid + 1;
        }
        if (n++ == 0) {
            a.add(new Sqrt());
        ...
        if (lo == a.size()) a.get(--lo).insert(val);
        else a.get(lo).insert(val);
        Sqrt alo = a.get(lo);
        int cbrtnsq = ((int) (Math.cbrt(n) * Math.cbrt(n))) * FACTOR;
        if (alo.n > 2 * cbrtnsq) {
            ArrayList<Integer> b = new ArrayList<Integer>(cbrtnsq);
            while (alo.n > cbrtnsq) {
                b.add(alo.getLast());
                alo.removeLast();
            }
            Collections.reverse(b);
            a.add(lo + 1, new Sqrt(b));
            prefixSZ.add(0);
        }
        for (int j = lo + 1; j < a.size(); j++) prefixSZ.set(j, prefixSZ.get(j - 1) + a.get(j - 1).n);
    }
    ...
    public static void remove(int val) {
        ...
        while (lo < hi) {
            mid = lo + (hi - lo) / 2;
            if (a.get(mid).getLast() < val) lo = mid + 1;
            else hi = mid;
        }
        if (lo == a.size()) return;
        Sqrt alo = a.get(lo);
        if (!alo.remove(val)) return;
        --n;
        if (alo.n == 0) {
            a.remove(lo);
            prefixSZ.remove(prefixSZ.size() - 1);
    ...
    public static int getRank(int val) {
        ...
        if (lo == a.size()) return -1;
        int r = a.get(lo).getRank(val);
        return r == -1 ? -1 : prefixSZ.get(lo) + r;
    }

    public static void print() {
        for (Sqrt b : a) b.print();
    }
