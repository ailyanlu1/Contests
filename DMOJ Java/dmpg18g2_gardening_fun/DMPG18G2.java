...
public class DMPG18G2 {
    private static DMPG18G2 o = new DMPG18G2();
    ...
    public static void run(int testCaseNum) throws Exception {
        ...
        long[][] dp = new long[2][MAXABC + 1];
        dp[0][0] = 0;
        for (int j = 1; j <= MAXABC; j++) dp[0][j] = INF;
        for (int i = 1; i <= N; i++) {
            long min = INF;
            for (int j = 0; j <= MAXABC; j++) {
                min = Math.min(min, dp[1 - i % 2][j] - B * j);
                dp[i % 2][j] = min + A * j + B * j + C * (V[i] - j) * (V[i] - j);
            }
            min = INF;
            for (int j = MAXABC; j >= 0; j--) {
                min = Math.min(min, dp[1 - i % 2][j]);
                dp[i % 2][j] = Math.min(dp[i % 2][j], min + A * j + C * (V[i] - j) * (V[i] - j));
            }
        }
        long ans = INF;
        for (int j = 0; j <= MAXABC; j++) ans = Math.min(ans, dp[N % 2][j]);
        out.println(ans);
    }
}
