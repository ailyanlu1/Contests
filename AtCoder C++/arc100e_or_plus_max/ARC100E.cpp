...
//    freopen("out.txt", "w", stdout);
    ...
    FOR(i, N) FOR(j, 1 << N) if ((j >> i) & 1) FOR(k, 2) for (int m = 0, x = dp[(1 << i) ^ j][k]; m < 2; m++) if (dp[j][m] < x) swap(dp[j][m], x);
    ans[0] = 0;
    For(i, 1, 1 << N) writeln(ans[i] = max(dp[i][0] + dp[i][1], ans[i - 1]));
    return 0;
}
