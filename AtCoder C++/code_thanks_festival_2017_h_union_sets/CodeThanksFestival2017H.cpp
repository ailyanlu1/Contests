...
int find(int u, int t) {
    return UF[u].back().f >= 0 && UF[u].back().s <= t ? find(UF[u].back().f, t) : u;
}
...
bool join(int u, int v) {
    ...
    v = find(v, curT);
    if (u == v) return false;
    if (UF[u].back().f > UF[v].back().f) swap(u, v);
    UF[u].eb(UF[u].back().f + UF[v].back().f, curT);
    UF[v].eb(u, curT);
    return true;
}

int search(int u, int v) {
    int lo = 0, hi = curT + 1, mid;
    while (lo < hi) {
        mid = lo + (hi - lo) / 2;
        if (find(u, mid) == find(v, mid)) hi = mid;
        else lo = mid + 1;
    }
    return lo == curT + 1 ? -1 : lo;
}
