...
//    freopen("out.txt", "w", stdout);
    ...
    for (int i = 1, j = 2, k = 3; j < N - 1 && k < N; j++) {
        while (i + 1 < j && abs(A[i] - (A[j] - A[i])) > abs(A[i + 1] - (A[j] - A[i + 1]))) i++;
        MAX(k, j + 1);
        while (k + 1 < N && abs((A[k] - A[j]) - (A[N] - A[k])) > abs((A[k + 1] - A[j]) - (A[N] - A[k + 1]))) k++;
        MIN(ans, max(max(A[i], A[j] - A[i]), max(A[k] - A[j], A[N] - A[k])) - min(min(A[i], A[j] - A[i]), min(A[k] - A[j], A[N] - A[k]))); 
    }
    writeln(ans);
    return 0;
}
