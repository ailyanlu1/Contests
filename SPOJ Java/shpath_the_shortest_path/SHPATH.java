...
public class SHPATH {
    private static SHPATH o = new SHPATH();
    ...
    ***************************************************************************/
    ...
    static int N, R;
    static int[] dist;
    static HashMap<String, Integer> ind;
    ...
    public static void run(int testCaseNum) throws Exception {
        ...
        for (int i = 0; i < N; i++) {
            ...
            int p = in.nextInt();
            for (int j = 0; j < p; j++) adj[i].add(new int[] {in.nextInt() - 1, in.nextInt()});
        }
        R = in.nextInt();
        for (int i = 0; i < R; i++) out.println(dijkstra(ind.get(in.next()), ind.get(in.next())));
    }
    
    static int dijkstra(int s, int t) {
    	IndexMinPQ<Integer> PQ = new IndexMinPQ<Integer>(N);
    	for (int v = 0; v < N; v++) dist[v] = 0x3f3f3f3f;
    	dist[s] = 0;
        PQ.insert(s, dist[s]);
        while (!PQ.isEmpty()) {
            int v = PQ.delMin();
