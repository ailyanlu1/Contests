...
public class TrainSorting {
    private static TrainSorting o = new TrainSorting();
    ...
    public static void run(int testCaseNum) throws Exception {
        ...
        for (int i = N - 1; i >= 0; i--) {
            dp[0][i] = 1;
            for (int j = i + 1; j < N; j++) if (A[i] < A[j]) dp[0][i] = Math.max(dp[0][i], dp[0][j] + 1);
        }
        for (int i = N - 1; i >= 0; i--) {
            dp[1][i] = 1;
            for (int j = i + 1; j < N; j++) if (A[i] > A[j]) dp[1][i] = Math.max(dp[1][i], dp[1][j] + 1);
        }
        int ans = 0;
        for (int i = 0; i < N; i++) ans = Math.max(ans, dp[0][i] + dp[1][i] - 1);
        out.println(ans);
    }
}
