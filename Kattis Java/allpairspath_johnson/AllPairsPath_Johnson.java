...
public class AllPairsPath_Johnson {
    ...
    public static void run(int testCaseNum) throws Exception {
        while (true) {
            ...
            for (int v = 0; v < N; v++) {
                h[v] = INF;
                adj[N].add(new int[]{v, 0});
            }
            for (int i = 0; i < M; i++) adj[in.nextInt()].add(new int[]{in.nextInt(), in.nextInt()});
            h[N] = 0;
            for (int i = 0; i < N; i++) for (int v = 0; v <= N; v++) for (int[] e : adj[v]) if (h[e[0]] > h[v] + e[1]) h[e[0]] = h[v] + e[1];
            boolean inCycle = true;
            while (inCycle) {
                inCycle = false;
                for (int v = 0; v <= N; v++) {
                    for (int[] e : adj[v]) {
                        if (h[e[0]] > -INF && h[e[0]] > h[v] + e[1]) {
                            h[e[0]] = -INF;
                            inCycle = true;
                        }
                    }
            ...
            for (int s = 0; s < N; s++) {
                ...
                for (int v = 0; v < N; v++) dist[s][v] = vis[v] && h[v] == -INF ? -INF : INF;
                if (h[s] == -INF) continue;
                IndexMinPQ<Integer> pq = new IndexMinPQ<Integer>(N);
                dist[s][s] = 0;
                pq.insert(s, dist[s][s]);
                while (!pq.isEmpty()) {
                    int v = pq.delMin();
                    for (int[] e : adj[v]) {
                        if (h[e[0]] == -INF) continue;
                        if (dist[s][e[0]] > dist[s][v] + e[1] + h[v] - h[e[0]]) {
                            dist[s][e[0]] = dist[s][v] + e[1] + h[v] - h[e[0]];
                            if (pq.contains(e[0])) pq.decreaseKey(e[0], dist[s][e[0]]);
                            else pq.insert(e[0], dist[s][e[0]]);
                        }
                    }
                }
            }
            for (int v = 0; v < N; v++) for (int w = 0; w < N; w++) if (Math.abs(dist[v][w]) < INF) dist[v][w] += h[w] - h[v];
            for (int i = 0; i < Q; i++) {
