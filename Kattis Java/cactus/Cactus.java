// https://open.kattis.com/problems/cactus
import java.io.*;
import java.math.*;
import java.util.*;
...
public class Cactus {
    ...
    static boolean dfs(int v, int prev) {
        ...
        for (int w : adj[v]) {
            if (pre[w] == -1) {
                ...
                if (pre[v] > low[w] && ++c > 1) return false;
                low[v] = Math.min(low[v], low[w]);
            } else {
                if (vis[w]) return false;
                if (pre[w] < low[v] && ++c > 1) return false;
    ...
    public static void run(int testCaseNum) throws Exception {
        ...
        if (dfs(0, 0) && cur == N) out.println("YES");
        else out.println("NO");
    }
}
