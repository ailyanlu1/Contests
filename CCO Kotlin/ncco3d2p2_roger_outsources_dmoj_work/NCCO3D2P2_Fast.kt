...
fun run(testCaseNum: Int) {
    ...
    val dp = Array(N + 1, {if (it == 0) Pair(1, 0) else Pair(0x3f3f3f3f.toInt(), 0x3f3f3f3f.toInt())})
    for (i in 1..N) {
        var cur = A[i]
        for (j in i - 1 downTo 0) {
            val x = Pair(dp[j].first + (if (M - cur.first < dp[j].second) 2 else 1), cur.second)
            if (x.first < dp[i].first || (x.first == dp[i].first && x.second < dp[i].second)) dp[i] = x
            cur = Pair(cur.first + A[j].first, cur.second + A[j].second)
            if (cur.first > M || cur.second > M) break
        }
    }
    Out.println(dp[N].first + 1)
}
