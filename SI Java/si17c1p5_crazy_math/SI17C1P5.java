...
public class SI17C1P5 {
    ...
    static Pair fib(long i) {
        if (i == 0) return new Pair(0, 1);
        Pair res = fib(i / 2);
        long x = res.first * (res.second * 2 - res.first + MOD) % MOD;
        long y = ((res.first * res.first) % MOD + (res.second * res.second) % MOD) % MOD;
        if (i % 2 == 0) return new Pair(x, y);
        return new Pair(y, (x + y) % MOD);
    }
    ...
    public static void run(int testCaseNum) throws Exception {
        ...
        long B = in.nextLong();
        long N = in.nextLong();
        if (N == 0) out.println(A);
        else if (N == 1) out.println(B);
        else out.println((fib(N - 1).first * A % MOD + fib(N).first * B % MOD) % MOD);
    }
}
