...
private:
    ...
    int query(Node *cur, int cL, int cR, int l, int r) {
        ...
        if (cL >= l && cR <= r) {
            ...
            cur->propogate();
            int m = cL + (cR - cL) / 2;
            ans += query(cur->left, cL, m, l, r);
            ans += query(cur->right, m + 1, cR, l, r);
            cur->merge();
        }
        return ans;
    }

    void update(Node *cur, int cL, int cR, int l, int r) {
            if (cL > r || cR < l) return;
            if (cL >= l && cR <= r) {
                cur->rotate(1);
            } else {
                cur->propogate();
                int m = cL + (cR - cL) / 2;
                update(cur->left, cL, m, l, r);
                update(cur->right, m + 1, cR, l, r);
                cur->merge();
            }
        }
