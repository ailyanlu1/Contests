// http://codeforces.com/problemsets/acmsguru/problem/99999/519
import java.io.*;
import java.math.*;
import java.util.*;
...
public class SGU519 {
    ...
    public static void run(int testCaseNum) throws Exception {
        ...
        int SA = 0;
        for (int i = 0; i < N + 2; i++) for (int j = 0; j < M + 2; j++) A[i][j] = 0;
        for (int i = 1; i <= N; i++) {
            String S = in.next();
            for (int j = 1; j <= M; j++) A[i][j] = S.charAt(j - 1) - '0';
        }
        for (int i  =1; i <= N; i++) for (int j = 1; j <= M; j++) SA += A[i][j] == 0 ? 0 : (A[i][j] * 4 + 2 - Math.min(A[i - 1][j], A[i][j]) - Math.min(A[i + 1][j], A[i][j]) - Math.min(A[i][j - 1], A[i][j]) - Math.min(A[i][j + 1], A[i][j]));
        out.println(SA);
    }
}
