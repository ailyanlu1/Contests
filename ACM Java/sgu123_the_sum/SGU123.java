// http://codeforces.com/problemsets/acmsguru/problem/99999/123
import java.io.*;
import java.math.*;
import java.util.*;

public class SGU123 {
    private static SGU123 o = new SGU123();
    ...
    public static void run(int testCaseNum) throws Exception {
        out.println(fib(in.nextInt() + 2)[0] - 1);
    }
    
    public static int[] fib(int n) {
        if (n == 0) return new int[] {0, 1};
        int[] res = fib(n / 2);
        int a = res[0] * (res[1] * 2 - res[0]);
        int b = res[0] * res[0] + res[1] * res[1];
        if (n % 2 == 0) return new int[] {a, b};
        else return new int[] {b, a + b};
    }
}
