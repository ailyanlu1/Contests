// http://codeforces.com/problemsets/acmsguru/problem/99999/133
import java.io.*;
import java.math.*;
import java.util.*;

public class SGU133 {
    private static SGU133 o = new SGU133();
    ...
    public static void run(int testCaseNum) throws Exception {
        int N = in.nextInt();
        Pair[] P = new Pair[N];
        for (int i = 0; i < N; i++) P[i] = new Pair(in.nextInt(), in.nextInt());
        Arrays.sort(P);
        int cnt = 0;
        int end = P[0].b;
        for (int i = 1; i < N; i++) {
            if (P[i].b > end) end = P[i].b;
            else cnt++;
        }
        out.println(cnt);
    }
