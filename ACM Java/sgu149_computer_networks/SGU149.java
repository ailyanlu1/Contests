// http://codeforces.com/problemsets/acmsguru/problem/99999/149
import java.io.*;
import java.math.*;
import java.util.*;
...
public class SGU149 {
    ...
    static void dfs(int v, int prev, int curDist, int[] dist, int[] best) {
        dist[v] = curDist;
        if (dist[v] > best[0]) {
            best[0] = dist[v];
            best[1] = v;
        }
        for (int[] e : adj[v]) if (e[0] != prev) dfs(e[0], v, curDist + e[1], dist, best);
    }
    ...
    public static void run(int testCaseNum) throws Exception {
        ...
        for (int i = 1; i < N; i++) {
            int v = in.nextInt() - 1;
            int d = in.nextInt();
            adj[i].add(new int[]{v, d});
            adj[v].add(new int[]{i, d});
        }
        int[] best = {-1, -1};
        dfs(0, -1, 0, dist1, best);
        best[0] = -1;
        dfs(best[1], -1, 0, dist1, best);
        best[0] = -1;
        dfs(best[1], -1, 0, dist2, best);
        for (int i = 0; i < N; i++) out.println(Math.max(dist1[i], dist2[i]));
    }
}
