...
public class APIO16P2_PQ_DSU {
    ...
    public static void run(int testCaseNum) throws Exception {
        ...
        PriorityQueue<Long>[] PQ = new PriorityQueue[N + M + 1];
        for (int i = 2; i <= N + M; inDeg[P[i] = in.nextInt()]++, C[i++] = in.nextInt());
        for (int i = 1; i <= N + M; i++) PQ[i] = new PriorityQueue<Long>(Collections.reverseOrder());
        for (int i = N + M; i >= 2; E[P[i]] += E[i] - C[i], i--) {
            ...
            if (PQ[i].size() > PQ[P[i]].size()) { PQ[0] = PQ[i]; PQ[i] = PQ[P[i]]; PQ[P[i]] = PQ[0]; }
            while (!PQ[i].isEmpty()) PQ[P[i]].offer(PQ[i].poll());
        }
        long ans = E[1];
