...
public class COCI07C2P6 {
    private static COCI07C2P6 o = new COCI07C2P6();
    public class Reader {
    ...
    private static class Line implements Comparable<Line> {
        ...
        public Line(long dx, long dy, int quadrant) {
            this.dx = dx;
            this.dy = dy;
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < N; j++) {
                if (i == j) continue;
                Line L = new Line((long) (points[j].x() - points[i].x()), (long) (points[j].y() - points[i].y()), 0);
                while (L.dx <= 0 || L.dy < 0) {
                    L.rotate90();
                }
                lines.add(L);
            ...
            int[] count = new int[4];
            for (int j = 0, k = 0; j < lines.size(); j = k) {
                count[0] = count[1] = count[2] = count[3] = 0;
                for (k = j; k < lines.size() && lines.get(j).equals(lines.get(k)); k++) {
                    count[lines.get(k).quadrant]++;
                }
