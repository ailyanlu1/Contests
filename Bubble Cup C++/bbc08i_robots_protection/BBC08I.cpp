...
void update(int a, int x, int y, int v) {
    for (int i = x; i <= MAXN; i += i & -i) for (int j = y; j <= (a == 0 ? MAXN : 2 * MAXN); j += j & -j) BIT[a][i][j] += v;
}

int rsq(int a, int x, int y) {
    int sum = 0;
    for (int i = x; i > 0; i -= i & -i) for (int j = y; j > 0; j -= j & -j) sum += BIT[a][i][j];
    return sum;
}
...
//    freopen("out.txt", "w", stdout);
    ...
    BIT = new int**[3];
    FOR(i, 3) {
        BIT[i] = new int*[MAXN + 1];
        FOR(j, MAXN + 1) {
            if (i == 0) {
                BIT[i][j] = new int[MAXN + 1];
                FOR(k, MAXN + 1) BIT[i][j][k] = 0;
            } else {
                BIT[i][j] = new int[MAXN * 2 + 1];
                FOR(k, 2 * MAXN + 1) BIT[i][j][k] = 0;
            }
        }
    }
    int t, d, x, y, L;
    FOR(q, Q) {
        ...
        if (t == 1) {
            ...
            if (L == 0) {
                ...
                update(0, x, y, 1);
                update(0, x + L + 1, y, -1);
                update(1, x, x + y + L + 1, -1);
                update(1, x + L + 1, x + y + L + 1, 1);
            } else if (d == 2) {
                update(0, x, y + 1, -1);
                update(0, x + L + 1, y + 1, 1);
                update(2, x, MAXN - x + y - L, 1);
                update(2, x + L + 1, MAXN - x + y - L, -1);
            } else if (d == 3) {
                update(0, x - L, y, 1);
                update(0, x + 1, y, -1);
                update(2, x - L, MAXN - x + y + L + 1, -1);
                update(2, x + 1, MAXN - x + y + L + 1, 1);
            } else {
                update(0, x - L, y + 1, -1);
                update(0, x + 1, y + 1, 1);
                update(1, x - L, x + y - L, 1);
                update(1, x + 1, x + y - L, -1);
            }
        } else {
            read(x, y);
            writeln(rsq(0, x, y) + rsq(1, x, x + y) + rsq(2, x, MAXN - x + y));
        }
    }
    return 0;
}
