...
ll ff(ll k, ll n) {
    ll N = (ll) (log(n * (k - 1) + 1) / log(k)) % MOD;
    ll x = (divMod((divMod((pow3(k, N + 2, MOD) + MOD - 1) % MOD, (k - 1) % MOD, MOD) - N - 2 + MOD) % MOD, (k - 1) % MOD, MOD) - (N * (N + 1) / 2 % MOD) - N - 1 + MOD + MOD + MOD) % MOD;
    x = divMod(x, (k - 1) % MOD, MOD);
    return x;
}
