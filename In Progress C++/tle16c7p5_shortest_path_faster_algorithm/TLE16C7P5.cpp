        ...
        while (!q.empty()) {
            ...
            for (FlowEdge *e : G->adj(v)) {
                ...
                if (level[w] < 0 && e->residualCapacityTo(w) > 0) {
                    ...
                    edgeTo[w] = v;
                    q.push(w);
                } else if (e->residualCapacityTo(w) > 0 && level[w] == level[v] + 1 && cmpMinVertex(v, edgeTo[w])) {
                    edgeTo[w] = v;
                    // TODO This means the min vertex of previously computed paths may change
                }
            }
        }
        if (cntF == queries[cntQ].f && level[t] != -1) {
            for (int x = t; x != -1; x = edgeTo[x]) {
                ans[queries[cntQ].s].push(x);
            }
            cntQ++;
        }
        cntF++;
...
int main() {
    ...
    FOR(i, M) {
        ...
        ri(v);
        rll(d);
        G->addEdge(new FlowEdge(u - 1, v - 1, d));
    }
    ri(Q);
    FOR(i, Q) {
        rll(queries[i].f); queries[i].f--;
        queries[i].s = i;
    }
    sort(queries, queries + Q);
    mf = new DinicMaxFlow(G, 0, N - 1);
    FOR(i, Q) {
        if (ans[i].empty()) printf("Fail\n");
        else {
            while (!ans[i].empty()) {
                printf("%d ", ans[i].top() + 1);
                ans[i].pop();
            }
            printf("\n");
        }
