...
int N, M, ON[MAXN + MAXM], OFF[MAXN + MAXM];
pii in[MAXM];
...
int main() {
    ...
    FOR(i, M) {
        ri(in[i].f); in[i].f--;
        ri(in[i].s); in[i].s--;
        ON[N + i] = !(ON[in[i].f] && ON[in[i].s]);
        OFF[N + i] = !(OFF[in[i].f] && OFF[in[i].s]);
    }
    int ON_OUT = ON[N + M - 1], OFF_OUT = OFF[N + M - 1];
    int bestCnt = INT_INF, bestPerm;
    int perm = pow(3, N);
    FOR(i, perm) {
        int p = i;
        int cnt = 0;
        ...
        FOR(j, M) {
            ON[N + j] = !(ON[in[j].f] && ON[in[j].s]);
            OFF[N + j] = !(OFF[in[j].f] && OFF[in[j].s]);
        }
        if (ON_OUT == ON[N + M - 1] && OFF_OUT == OFF[N + M - 1] && cnt < bestCnt) {
            bestCnt = cnt;
            bestPerm = i;
        }
    }
    FOR(i, N) {
        if (bestPerm % 3 == 0) printf("0");
        else if (bestPerm % 3 == 1) printf("1");
        else printf("x");
        bestPerm /= 3;
    }
    printf("\n");
