...
int N, M, P;
bool hasType[MAXNMP + 1];

int main() {
    ri(N);
    ri(M);
    ri(P);
    G = new FlowNetwork(N + M + P + 2);
    int source = 0;
    int sink = N + M + P + 1;
    for (int i = 1; i <= N; i++) {
        G->addEdge(new FlowEdge(source, i, 1)); // source to child
    }
    for (int i = 1; i <= N; i++) {
        int k;
        ri(k);
        for (int j = 0; j < k; j++) {
            int ik;
            ri(ik);
            G->addEdge(new FlowEdge(i, N + ik, 1)); // child to toy
        }
    }
    for (int i = 1; i <= P; i++) {
        int l;
        ...
        for (int j = 0; j < l; j++) {
            int t;
            ri(t);
            G->addEdge(new FlowEdge(N + t, N + M + i, 1)); // toy to type
            hasType[t] = true;
        }
        int r;
        ri(r);
        G->addEdge(new FlowEdge(N + M + i, sink, r)); // type to sink
    }
    for (int i = 1; i <= M; i++) {
        if (!hasType[i]) G->addEdge(new FlowEdge(N + i, sink, 1)); // no type to sink
    }
    mf = new EdmondsKarpMaxFlow(G, source, sink);
    printf("%d", (int) (mf->getValue()));
    return 0;
}
